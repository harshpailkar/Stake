...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        let isStackingPaused = await cObj.methods.getStakingStatus().call();
        let isStackingPausedText;
        ...
        let stakeDays = await cObj.methods.ggetStakeDays().call();
        ...
        if(isStackingPaused) {
            isStackingPausedText = "Paused";
        } else if (currentDate < startDate) {
            isStackingPausedText = "Locked";
        } else if (currentDate > endDate) {
            isStackingPausedText = "Ended"
        } else {
            isStackingPausedText = "Active";
        }

        document.querySelectorAll(".active-status-stacking").forEach(function (element) {
            element.innerHTML = `${isStackingPausedText}`;
        });
...
async function stackTokens() {
    try {
        ...
        if(balMainUser < nTokens){
            notyf.error(`insufficient token on ${SELECT_CONTRACT[_NETWORK_ID].network_name}.\nPlease buy some tokens first!`);
            return;
        }
...
async function approveTokenSpend(_mint_fee_wei, sClass) {
    ...
    try {
        gasEstimation = await oContractToken.methods.approve(SELECT_CONTRACT[_NETWORK_ID].STACKING[sClass].address, _mint_fee_wei).estimateGas({from: currentAddress, });
    } catch (error) {
        console.log(error);
        notyf.error(formatEthErrorMsg(error));
        return;
    ...
    oContractToken.methods.approve(SELECT_CONTRACT[_NETWORK_ID].STACKING[sClass].address, _mint_fee_wei).send({from: currentAddress, gas: gasEstimation, }).on("transactionHash", (hash) => {
...
async function stackTokenMain(_amount_wei, sClass) {
    ...
    try {
        gasEstimation = await oContractStacking.methods.stake(_amount_wei).estimateGas({from: currentAddress, });
    } catch (error) {
        console.log(error);
        notyf.error(formatEthErrorMsg(error));
        return;
    ...
    oContractStacking.methods.stake(_amount_wei).send({from: currentAddress, gas: gasEstimation, }).on("receipt", (receipt) => {
        console.log(receipt);
        const receiptObj = {
            token: _amount_wei,
            from: receipt.from,
...
async function unstackTokens() {
    try {
        ...
        let balMainUser = await oContractToken.methods.getUser(currentAddress).call();
...
async function unstackTokenMain(_amount_wei, oContractStacking, sClass) {
    ...
    try {
        gasEstimation = await oContractStacking.methods.unstake(_amount_wei).estimateGas({from: currentAddress, });
    } catch (error) {
        console.log(error);
        notyf.error(formatEthErrorMsg(error));
        return;
    ...
    oContractStacking.methods.unstake(_amount_wei).send({from: currentAddress, gas: gasEstimation, })
    .on("receipt", (receipt) => {
        console.log(receipt);
        const receiptObj = {
            token: _amount_wei,
...
async function claimTokens() {
    try {
        ...
        let rewardBal = await oContractStacking.methods.getUserEstimatedRewards().call({from: currentAddress});
...
async function claimTokenMain(oContractStacking, sClass) {
    ...
    try {
        gasEstimation = await oContractStacking.methods.claimReward().estimateGas({from: currentAddress, });
    ...
    oContractStacking.methods.claimReward().send({from: currentAddress, gas: gasEstimation, })
    .on("receipt", (receipt) => {
        console.log(receipt);
        const receiptObj = {
            token: _amount_wei,
