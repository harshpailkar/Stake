...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
    ...
    function __TokenStaking_init_unchained(
        ...
        require(stakeDays_ > 0, "ToeknStaking: stake days must be non-zero");
        require(tokenAddress_ != address(0), "TokenStaking: token address cannot be 0 address");
        require(stakeStartDate_ < stakeEndDate_, "ToeknStaking: start date must be less than end date");
    ...
    function getTotalStakeToekns() external view returns (uint256) {
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        ...
        _users[user_].lastStakingTime = currentTime;
    ...
    function unstake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount) {
        ...
        if(getCurrentTime() <= _users[user].lastStakingTime + _stakeDays) {
    ...
    function _getUserEstimatedRewards(address _user) private view returns (uint256, uint256) {
        ...
        if(currentTime > _users[_user].lastStakingTime + _stakeDays) {
            currentTime = _users[_user].lastStakingTime + _stakeDays;
        }
